...
1. Gyro Settings: ```CAN_PIDGEOTTO``` and ```VAL_PIDGEOTTO_INV``` (ensure that the gyro rotation is CCW+ (Counter Clockwise Positive)
2. ```chosenModule```: 
...
3. ```VAL_TRACK_WIDTH```: Center to Center distance of left and right modules in meters.
4. ```VAL_WHEEL_BASE```: Center to Center distance of front and rear module wheels in meters.
5. ```VAL_WHEEL_CIRCUM```: Cirumference of the wheel (including tread) in meters. <br><b>If you are using a supported module, this value will be automatically set.</b>
6. ```VAL_DRIVE_GEAR_RATIO```: Total gear ratio for the drive motor. <br><b>If you are using a supported module, this value will be automatically set.</b>
7. ```VAL_STEER_GEAR_RATIO```: Total gear ratio for the angle motor. <br><b>If you are using a supported module, this value will be automatically set.</b>
8. ```VAL_CANCODER_INV``` and ```VAL_STEER_MOTOR_INV```: Both must be set such that they are CCW+. <br><b>If you are using a supported module, this value will be automatically set.</b>
9. ```VAL_DRIVE_MOTOR_INV```: <b>If you are using a supported module, this value will be automatically set.</b>
...
11. Setting Offsets
    ...
    * Point the bevel gears of all the wheels in the same direction (either facing left or right), where a postive input to the drive motor drives the robot forward (you can use phoenix tuner to test this). If for some reason you set the offsets with the wheels backwards, you can change the ```VAL_DRIVE_MOTOR_INV``` value to fix.
    * Open smartdashboard (or shuffleboard and go to the smartdashboard tab), you will see 4 printouts called "Mod 0 Cancoder", "Mod 1 Cancoder", etc. 
    <br>If you have already straightened the modules, copy those 4 numbers exactly (to 2 decimal places) to their respective ```VAL_ANGLE_OFFSET``` variable in constants.
    <br><b>Note:</b> The CANcoder values printed to smartdashboard are in degrees, when copying the values to ```VAL_ANGLE_OFFSET``` you must use ```Rotation2d.fromDegrees("copied value")```.
...
13. ```VAL_MAX_SPD```: In Meters Per Second. ```VAL_MAX_ANG_VELO```: In Radians Per Second. For these you can use the theoretical values, but it is better to physically drive the robot and find the actual max values.
...
<br>Leave ```VAL_DRIVE_KI```, ```VAL_DRIVE_KD```, and ```VAL_DRIVE_KF``` at 0.0.
